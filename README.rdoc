...
- change username to system name in config/database.yml
- rake db:create:all
- rails s
- clean gemfile
.ensure it has jquery, sass, and maybe haml
.delete coffee gem
- add scaffold for Resource model
.rails g scaffold Resource title:string raw_url:string clean_url:string description:text
- add user_id column to Resource model
.generate the migration - rails g migration AddUserIdToResource
.edit migration
.rake db:migrate
- routes for root
.added to routes: root :to => 'resources#index'
.deleted public/index.html
- add columns to resource model for keywords, keywords from site, title from site, description from site
.rails g migration AddColumnsToResource
.manually change migration file, rake db:migrate
...
.has_many resources, :through => :
resources has_many :collections, :through => :collectionizes
resources has_many :collectionizes
.joins table - .collection_id:integer, resource_id:integer, order:integer, instruction:text
